...
| Phase 14 unified the two species-correction write paths into a single `species_corrections` table (`UNIQUE(detection_id)` + UPSERT), replacing read-time precedence with write-time recency (D-03) — Gallery-vs-video-player precedence is now decided by whichever correction was written most recently, not by a fixed "video-player always wins" ordering. Plan 14-01 additionally added a `suppressed INTEGER` column beyond RESEARCH.md's recommended DDL (Claude's discretion), so a video-player suppress sentinel is a structural boolean rather than an overloaded `corrected_label IS NULL` encoding | RESEARCH.md's proposed `corrected_label IS NULL` suppress encoding would collide with the Gallery path's own NULL `corrected_label` (Gallery never collects a formal taxonomy label), making one NULL value mean two different things depending on `source` — a dedicated column removes that ambiguity structurally and matches this schema's existing boolean-flag pattern (`videos.kept`, `videos.has_animal`, `needs_reprocess`) | ✓ Good — pinned by plan 14-01's `unified`/`audit` suites (12/12); D-07 (below) tracks the legacy tables' eventual removal |
| Phase 14 (D-06): `species.user_common_name`/`user_scientific_name`/`corrected_at` and the entire `video_corrections` table are frozen read-only from cutover forward — readable (audit trail, `get_video_corrections()`), never written by any code path. D-07: removal of these frozen legacy columns/table is deferred to a future follow-up phase once `species_corrections` has run cleanly in production for a while — not scheduled automatically, no deadline set | Freezing rather than dropping in the same phase that introduces the new table keeps the cutover reversible during its highest-risk window (first production exposure); an explicit future removal phase avoids silently deleting the CORR-04 audit-trail source before its replacement is proven | ✓ Accepted — tracked as `.planning/todos/pending/2026-08-21-legacy-correction-column-removal.md`, no deadline |
